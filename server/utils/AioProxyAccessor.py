...
class aoiAccessor():
    ...
    async def get_G_data(self, gid, token, cache, countOnly):
        ...
        try:
            ...
            raise makeTrackableException(
                e, f"get_G_data({gid}, {token}, {cache})")
    ...
    async def getComments(self, gid, token, fetchAll=False):
        try:
            if fetchAll:
                html = await self.getHtml(f"https://exhentai.org/g/{gid}/{token}/?hc=1#comments", cache=False)
            else:
                html = await self.getHtml(f"https://exhentai.org/g/{gid}/{token}/?p=0", cache=True)
    ...
    def checkGalleryImageLocal(self, gid, token, index) -> str:  # index从一开始 仅检测本地是否存在
    ...
    async def getGalleryImage(self, gid, token, index) -> str:  # index 从一开始
        ...
        try:
            ...
            skipHathKeyViewPageHtml = await self.getHtml(skipHathKeyViewPageUrl, cache=True)
    ...
    async def addListDownload(self, gidList: list) -> None:  # 有error 直接throw
        ...
        for gidListBlock in splittedList:
            ...
            logger.info(
                f"add list download G_DATA fetch {count}/{len(splittedList)}")
        for (gid, token) in gidList:
            assert int(gid) in g_data_map
        [self.loop.create_task(self.downloadManagerInstance.addDownload(
            int(gid), token, g_data_map[int(gid)])) for (gid, token) in gidList]
    ...
    async def continueDownload(self) -> int:
        ...
        [self.loop.create_task(self.downloadManagerInstance.addDownload(
            int(gid), token, g_data_map[int(gid)])) for (gid, token) in unFinishList]
    ...
    async def getGalleryPreviewIgnoreError(self, gid, token, index):
        try:
            return await self.getGalleryPreview(gid, token, index)
        except Exception as e:
    ...
    async def getPreviewSrcIndexMap(self, gid, token):  # 获取src对应index的映射
        ...
        tasks = [asyncio.create_task(self.getGalleryPreviewIgnoreError(
            gid, token, i+1)) for i in range(filecount)]

        srcList = await asyncio.gather(*tasks)
        return {src: {"index": i+1, "gid": gid, "token": token, } for i, src in enumerate(srcList) if src != None}

    async def checkParents(self, gid, token):  # 检查是否有已下载的父画廊 返回最近的 无则空
        if self.db.g_data[gid]:
    ...
    async def parentImgMultiplex(self, gid, token):  # 父画廊文件复用
        try:
            ...
            if parents:
                ...
                old = await self.getPreviewSrcIndexMap(parents[0]["gid"], parents[0]["token"])
                needDownload = []
                for url in new:
                    if url in old:
                        src = await self.getGalleryImage(old[url]["gid"], old[url]["token"], old[url]["index"])
                        dst = path_join(
                            self.cachePath, f"{gid}_{token}_{new[url]['index']:08d}.jpg")
                        if not os.path.exists(dst):
                            shutil.copy(src, dst)
                    else:
                        needDownload.append(new[url]["index"])
                logger.info(
                    f"复用父画廊文件 {len(new) - len(needDownload)}/{len(new)}")
    ...
    async def updateFatherTree(self):  # 更新father_tree表 删除旧版本画廊
        ...
        need_update_gid_list = [
            gid for gid in self.db.father_tree.keys()
    ...
    async def syncFromOtherServer(self, notify: Coroutine, recv: Coroutine):
        ...
        try:
            remoteDownloadData = await (await self.httpGet(targetServerUrl + "/api/listAllDownload")).json()
        except Exception as e:
        ...
        async def downloader():
            ...
            while True:
                ...
                async with lock:
                    current = 100 * \
                        (totalTaskNum - len(taskList)) // totalTaskNum
    ...
    def getHistory(self):
        return sorted(self.db.history.values(), key=lambda x: x['timestamp'], reverse=True)
